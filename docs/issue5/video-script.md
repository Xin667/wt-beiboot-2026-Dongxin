...
| 4 | Tests & CI | ~2 min | `npm test` (51 Tests) laufen lassen; `npm run check`; Blick auf `.github/workflows/deploy.yml` (Test-Gate vor Deploy). |
| 5 | Deployment | ~1 min | Die öffentliche Pages-URL öffnen (`…/demo/`), Kamera freigeben, eine Geste auslösen. |
| 6 | **Showcase: Synthesizer** | ~1 min | `…/synth/` öffnen: peace startet den Looper, 👍/👎 improvisieren, Hand-Skelett-Overlay und Spektrum laufen live mit. Zeigt die Library als nutzbare Abstraktion (gleiche Gesten, andere Bedeutung als in `demo/`). |
| 7 | Lokaler Start | ~1 min | `npx serve src` bzw. VS Code Go Live zeigen. |
| 8 | Zusammenfassung | ~1 min | Was geändert wurde (ADR 0007), Trade-off (150 ms Latenz vs. Stabilität), Ausblick. |
...
- Abschnitt 6 ist der stärkste „Wow"-Teil: Der Synthesizer reagiert unmittelbar und sichtbar (Spektrum, Skelett) – hier lässt sich gut zeigen, dass dieselbe Library in einer anderen App völlig andere Interaktionen treibt.
